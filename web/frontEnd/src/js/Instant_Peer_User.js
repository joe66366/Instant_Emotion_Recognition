...
if(Modeselect==false){
	...
	changemode.src="../src/image/mode_auto.png";
	 $('#demo div').remove();
	setTimeout(function(){do_cutfull();},3000);
	intervalID = setInterval(function(){do_cutfull();},10000);
}
}
...
Dvc1.onclick=function(){
	document.getElementById("Create-peer").hidden = false;
	if(Language===false){
		document.getElementById("Menu").innerHTML="拍攝方";
	}
	else{
		document.getElementById("Menu").innerHTML="Filming Device";
	}
	Device_select=false;
}

Dvc2.onclick=function(){
	document.getElementById("Create-peer").hidden = false;
	if(Language===false){
		document.getElementById("Menu").innerHTML="操控方";
	}
	else{
		document.getElementById("Menu").innerHTML="Controlling Device";
	}
	Device_select=true;
}

Menu_Start.onclick=function(){
	document.getElementById("t1").hidden = true;
	let room = document.getElementById("room-input").value;
		if(Device_select===false){
			createRoom();
		}
		else if (Device_select===true){
			joinRoom();
		}
	if (room == " " || room == "") {
		return;
		...
		document.getElementById("Dvc1").hidden = true;
		document.getElementById("Dvc2").hidden = true;
...
function Start_doing(){
	...
	var menu_room=document.getElementById("room-input");
	var choice_title=document.getElementById("t1");
	var choice_ca=document.getElementById("Dvc1");
	var choice_co=document.getElementById("Dvc2");
	var choice_back=document.getElementById("Device");
	
	document.body.style.backgroundColor =Backgroud_Color ;
	//document.getElementById("Creat-peer").style.backgroundColor=Backgroud_Color;
	menu_room.style.border="2px solid "+Text_Color;
		
	menu_start.style.backgroundColor=Backgroud_Color;
	menu_room.style.backgroundColor=Backgroud_Color;
	choice_back.style.backgroundColor =Backgroud_Color ;
	choice_ca.style.backgroundColor=Backgroud_Color;
	choice_co.style.backgroundColor=Backgroud_Color;
	
	choice_title.style.color=Text_Color;
	choice_ca.style.color=Text_Color;
	choice_co.style.color=Text_Color;
	menu.style.color=Text_Color;
	menu_start.style.color=Text_Color;
	menu_room.style.color=Text_Color;
	
	choice_title.style.fontSize=Text_Size+"px";
	choice_ca.style.fontSize=Text_Size+"px";
	choice_co.style.fontSize=Text_Size+"px";
	menu.style.fontSize=Text_Size+"px";
	menu_start.style.fontSize=Text_Size+"px";
	menu_room.style.fontSize=Text_Size+"px";
	
	if(Language===false){	
		choice_title.innerHTML="請選擇使用方式";
		choice_ca.innerHTML="拍攝方";
		choice_co.innerHTML="操控方";
		menu_start.innerHTML="開始";
		menu_room.placeholder="請輸入房間id";
	}
}
...
function createRoom() {
	...
	document.getElementById("changecamera").hidden = false;
    room_id = PRE + room + SUF;
    peer = new Peer(room_id)
    peer.on('open', (id) => {
        console.log("Peer Connected with ID: ", id)
        /*hideModal()*/
        getUserMedia({ video:{facingMode: set_Camera }}, (stream) => {
            local_stream = stream;
            set_data_Stream(local_stream)
        }, (err) => {
            console.log(err)
        })
		if(Language===true){
...
function joinRoom() {
	...
	document.getElementById("changesound").hidden = false;
	document.getElementById("changetext").hidden = false;
	document.getElementById("download").hidden = false;
	StartProgram();
}

function set_data_Stream(stream) {
...
function hideModal() {
    document.getElementById("Create-peer").hidden = true
}
...
function notify(msg) {
	...
	notification.style.backgroundColor=Backgroud_Color;
	notification.style.color=Text_Color;
	notification.style.fontSize=Text_Size+"px";
